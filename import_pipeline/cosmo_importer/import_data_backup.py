...
class CosmoImporter(BaseImporter):
    ...
    def import_row(self, row):
        ...
        sample.groupId=row.loc['Group-ID']
        sample.reference=row.loc['Publication']
        sample.type=row.loc['Sample-type']
        sample.sampleYear=row.loc['Sampl-year']
    ...
    def measured_parameters(self, session, row):
        ...
        v = row.loc['Shield']
        val = self.datum(analysis,"Shield", v)
        dc.append(val)
